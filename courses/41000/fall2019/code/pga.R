...
tb = data.frame(Name=d00[ind,1], Money=d00[ind,3], Predicted=model00$fitted.values[ind], Error=model00$residuals[ind])
...
tb = data.frame(Name=d18[ind,1], Money=d18[ind,3], Predicted=model18$fitted.values[ind], 
...
par(mar = c(0,1,0,0))
plot(d[,-1], xaxt='n', ann=FALSE, yaxt='n', bg="lightblue", cex=0.7)
...
stargazer((d00 %>% arrange(res) %>% as.data.frame())[1:5,] %>% select(name, money, pred, res) %>% as.data.frame(), summary = F, rownames = F, digits = 0)
