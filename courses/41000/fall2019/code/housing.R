...
for (d in 1:maxdeg)
{
  ...
  for (i in 1:10){
    train_ind = sample(x=1:(dim(Boston)[1]),replace = F, size = n)
    training  = Boston[train_ind,]
    testing  =  Boston[-train_ind,]
    ...
    coeff[,i] = as.numeric(model$coefficients)
  }
  coeff_var = 0
  for (k in 2:10){
    for (j in 1:(k-1)){
      # print(c(k,j))
      print(mean(abs(coeff[,k] - coeff[,j])/coeff[,k]))
      coeff_var = coeff_var + mean(abs(coeff[,k] - coeff[,j])/abs(coeff[,k]))
    }
  }
  var[d] = coeff_var/d
  ous[d] = mse_ous/10
...
lines(testing$lstat[oo],predict(model,newdata = testing)[oo],col="red",lwd=3)
