...
lcex = 2
par(cex.lab=lcex,cex.main=lcex,cex.sub=lcex,cex.axis=lcex, mar=c(4.2,4.6,0,1),bty="n",cex=1,pch=21)
...
utils$fortify.acf <- function(acf){
  ...
  type <- switch(acf$type, correlation = "ACF", covariance = "ACF (cov)", 
                 partial = "Partial ACF")
  data.frame(acf = acf$acf[acf$n.used](range), lag =  acf$lag[](range), type = type, n.used =)
}
